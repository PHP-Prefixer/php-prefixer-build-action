/* Based on the git command manager by GitHub/ 'github-checkout/lib/git-command-manager' */
...
import * as refHelper from 'github-checkout/lib/ref-helper'
import * as regexpHelper from 'github-checkout/lib/regexp-helper'
import * as retryHelper from 'github-checkout/lib/retry-helper'
import {GitVersion} from 'github-checkout/lib/git-version'
...
export interface IGitCommandManager {
  ...
  tagSearch(pattern: string): Promise<string[]>
...
class GitCommandManager {
  ...
  async branchContainsTag(tag: string): Promise<string> {
    const output = await this.execGit(['branch', '--contains', `tags/${tag}`])

    return output.stdout.trim()
  }

  async pull(remote: string, branch: string): Promise<void> {
    const args = ['pull', '--progress', '--rebase', '--no-stat', remote, branch]
  ...
  async branchShowCurrent(): Promise<string> {
  ...
  async date(ref: string): Promise<Date> {
    const output = await this.execGit([
      'show',
      '--no-patch',
      '--no-notes',
  ...
  async remoteExists(remote: string): Promise<boolean> {
    const output = await this.execGit(['remote', 'show'])
    return output.stdout.trim().includes(remote)
  }
  ...
  async tagSearch(pattern: string): Promise<string[]> {
    const output = await this.execGit(['tag', '--list', pattern])
